...
export const MatrimonyBiodataDetailModal: React.FC<MatrimonyBiodataDetailModalProps> = ({
  ...
  const d = getMatrimonyDisplay(biodata, lang);
  const cleanNumber = (biodata.whatsappNumber || biodata.guardianPhone).replace(/[^0-9]/g, '');

  const waMessage = encodeURIComponent(
    lang === 'en'
      ? `Assalamu Alaikum. Contacting regarding the matrimonial profile "${d.codeName}" (${biodata.type === 'groom' ? 'Groom' : 'Bride'}) on the Barak Islamic App. We would like to discuss further for Islamic marriage proposal.`
...
আমরা বিস্তারিত আলোচনা ও দ্বীনি প্রস্তাবের জন্য কথা বলতে আগ্রহী।`
  ...
  const handleShare = () => {
    const shareTitle = `${lang === 'en' ? (biodata.type === 'groom' ? 'Groom' : 'Bride') : biodata.type === 'groom' ? 'পাত্র' : 'পাত্রী'}: ${d.codeName}`;
    const shareText = `${d.codeName} - ${lang === 'en' ? 'Age' : 'বয়স'}: ${biodata.age}, ${lang === 'en' ? 'Profession' : 'পেশা'}: ${d.profession}, ${lang === 'en' ? 'District' : 'জেলা'}: ${d.district}.`;
    if (navigator.share) {
      navigator.share({
        title: shareTitle,
        text: shareText,
      ...
      navigator.clipboard.writeText(
        `${d.codeName} | ${lang === 'en' ? 'Age' : 'বয়স'}: ${biodata.age} | ${lang === 'en' ? 'Profession' : 'পেশা'}: ${d.profession} | ${lang === 'en' ? 'District' : 'জেলা'}: ${d.district} | ${lang === 'en' ? 'Guardian Phone' : 'অভিভাবকের ফোন'}: ${biodata.guardianPhone}`
      );
      alert(lang === 'en' ? 'Biodata summary copied to clipboard!' : 'বায়োডাটার সারসংক্ষেপ কপি করা হয়েছে!');
    }
  };
      ...
      <div className="bg-[#03221F] border border-emerald-700/80 w-full max-w-xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-4 text-xs">
          ...
          <div className="grid grid-cols-3 gap-2 bg-[#072a25] p-3 rounded-xl border border-emerald-800/60 text-center">
            <div>
              <span className="text-[10px] text-emerald-400/80 block">{lang === 'en' ? 'Age' : 'বয়স'}</span>
              <span className="text-sm font-bold text-white">{biodata.age} {lang === 'en' ? 'Years' : 'বছর'}</span>
            </div>
            <div>
              <span className="text-[10px] text-emerald-400/80 block">{lang === 'en' ? 'Height' : 'উচ্চতা'}</span>
              <span className="text-sm font-bold text-white">{biodata.height}</span>
            </div>
            <div>
              <span className="text-[10px] text-emerald-400/80 block">{lang === 'en' ? 'Complexion' : 'গায়ের রঙ'}</span>
              <span className="text-sm font-bold text-[#E2A336]">{d.complexion}</span>
            </div>
          </div>
          ...
          <div className="space-y-2.5 bg-[#072a25] p-3.5 rounded-xl border border-emerald-800/60">
            <div className="flex items-start gap-2.5">
              ...
              <div>
                ...
                <p className="text-xs font-semibold text-white mt-0.5">{d.education}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-2.5 pt-2 border-t border-emerald-800/40">
              ...
              <div>
                <span className="text-[11px] text-emerald-400 font-medium">
                  {lang === 'en' ? 'Profession & Income:' : 'পেশা ও আয়:'}
                </span>
                <p className="text-xs font-semibold text-white mt-0.5">
                  {d.profession} {d.monthlyIncome ? `(${d.monthlyIncome})` : ''}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-2.5 pt-2 border-t border-emerald-800/40">
              ...
              <div>
                <span className="text-[11px] text-emerald-400 font-medium">
                  {lang === 'en' ? 'Address & District:' : 'ঠিকানা ও এলাকা:'}
                </span>
                <p className="text-xs font-semibold text-white mt-0.5">
                  {d.area}, {d.district}
                </p>
          ...
          <div className="space-y-1.5">
            <h4 className="text-xs font-bold text-emerald-200 flex items-center gap-1.5">
              ...
              <span>{lang === 'en' ? 'Religious Practices & Islamic Values:' : 'দ্বীনি বৈশিষ্ট্য ও ধর্মীয় অনুশাসন:'}</span>
            </h4>
            <div className="flex flex-wrap gap-1.5">
              {d.religiousPractices.map((prac, i) => (
                <span
                  key={i}
          ...
          <div className="space-y-1.5 bg-[#072a25] p-3 rounded-xl border border-emerald-800/60">
            <h4 className="text-xs font-bold text-emerald-200">
              {lang === 'en' ? 'Family Background:' : 'পারিবারিক পরিচিতি:'}
            </h4>
            <p className="text-xs text-emerald-100">
              <span className="text-emerald-400 font-medium">
                {lang === 'en' ? "Father's Profession:" : 'পিতার পেশা:'}
              </span>{' '}
              {d.fatherOccupation}
            </p>
            <p className="text-xs text-emerald-100">
              <span className="text-emerald-400 font-medium">
                {lang === 'en' ? 'Family Type:' : 'পরিবারের ধরন:'}
              </span>{' '}
              {d.familyType}
            </p>
            {d.bioNotes && (
          ...
          <div className="space-y-1.5 bg-[#072a25] p-3 rounded-xl border border-emerald-800/60">
            <h4 className="text-xs font-bold text-[#E2A336] flex items-center gap-1.5">
              ...
              <span>{lang === 'en' ? 'Partner Expectations:' : 'প্রত্যাশিত জীবনসঙ্গী:'}</span>
            </h4>
            <p className="text-xs text-emerald-100 leading-relaxed">
              {d.partnerExpectations}
            </p>
          </div>
          ...
          <div className="p-4 bg-[#09332E] border border-emerald-700/80 rounded-2xl space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <span className="text-[10px] text-emerald-400 font-medium">
                  {lang === 'en' ? 'Contact Guardian:' : 'যোগাযোগের অভিভাবক:'}
                </span>
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-emerald-800/60">
              ...
              >
                ...
                <span>{lang === 'en' ? 'WhatsApp Message' : 'WhatsApp বার্তা'}</span>
              </a>
              ...
              >
                ...
                <span>{lang === 'en' ? 'Call Directly' : 'সরাসরি কল করুন'}</span>
              </a>
            </div>
          </div>
        </div>
